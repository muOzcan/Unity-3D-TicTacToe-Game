...
public class oyunKodu : MonoBehaviour
{
    ...
    {// 0 -> boş, 1 -> x, 2 -> o
        ...
        for (int i = 0; i < 5; i++)
        {
            ...
            rbx.MovePosition(new Vector3(xSignPositions[i].x, 1f, xSignPositions[i].z));
            Rigidbody rbo = GameObject.Find("o" + (i + 1)).GetComponent<Rigidbody>();
            rbo.MovePosition(new Vector3(oSignPositions[i].x, 1f, oSignPositions[i].z));
        }
    }
    ...
    void Update()
    {
        ...
        if (squareNumber >= 0 && squareNumber < 9)
        {
            if (isSquareEmpty(squareNumber))
            {
                ...
                if (playerOrder == 1)
                {
                    Rigidbody xitem = getPlayerSignRigidbody(xOrder);
                    xitem.MovePosition(new Vector3(getSquarePosition(squareNumber).x, 1f, getSquarePosition(squareNumber).z));
                ...
                {
                    Rigidbody oitem = getPlayerSignRigidbody(oOrder);
                    oitem.MovePosition(new Vector3(getSquarePosition(squareNumber).x, 1f, getSquarePosition(squareNumber).z));
